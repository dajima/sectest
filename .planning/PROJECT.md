...
Sectest 是一个面向安全渗透测试人员的 AI 驱动安全审计平台，统一融合 SAST（静态代码审计）和 DAST（动态渗透测试）能力。平台基于多 Agent 协作架构，在 Kali Linux Docker 沙箱中运行完整的安全工具链，通过顺序流水线（侦察→分析→验证→报告）驱动主体流程，同时支持动态 Agent 编排查进行开放式深度探索。
...
- **Sectest (usestrix/strix)**: Apache 2.0 许可，openai-agents SDK 架构的 AI 渗透测试工具。核心优势：动态多 agent 协作用图、Docker 沙箱运行时、完整黑客工具包（HTTP 代理、浏览器自动化、终端、Python 运行时）、CLI/TUI 界面。22.8k GitHub stars。
...
**两者互补关系：**
- Sectest 擅长运行时动态测试（IDOR、权限绕过、CSRF、业务逻辑）但对代码级语义分析较弱
